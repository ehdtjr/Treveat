...
Future<List<model_review>> fetchReview(String loc_code) async {
  final response = await http.get(Uri.parse(
      'http://dongseok1.dothome.co.kr/treveat/mReview/selectValue.php?start=0&end=20&rsort=desc&loc_code=$loc_code'));
...
class _riview_grade extends State<riview_grade> {
  late Future<List<model_review>>
  futurereview; //late : non-nullable 변수의 초기화를 나중에 할 수 있게해줌

  @override
  void initState() {
    super.initState();
    futurereview =
        fetchReview(widget.loc_code_); //futureAlbum이 빌드시 fetchAlbum()으로 초기화 됨
  }
      ...
      body: Center(
        child: FutureBuilder(
          ...
          builder: (context, snapshot) {
            //snapshot 정보를 통해 비동기 처리 결과를 어들 수 있음
            if (snapshot.hasData) {
                  ...
                  child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int i) {
                        return Card(
                          ...
                          color: Colors.white,
                          child: (() {
                            if (_review[i].token_key == token) {//로그인한 사람의 토큰키변수 가져와서 넣기
                              return ListTile(
